...
const DEFAULT_DIRECT_INVOICE_RATE = 12.5
const DEFAULT_STAFFING_INVOICE_RATE = 12.5
const DEFAULT_CONSOLIDATED_INVOICE_RATE = 12.5
...
export async function createWeeklyAttendanceInvoice(
  ...
  if (!lines.length) {
    throw new Error("No approved attendance lines are available for this invoice.")
  }

  const staffingCompanyIds = [
    ...new Set(
      lines
        .map((line) => line.staffingCompanyId)
  ...
  const groups = [
    {
      type: WeeklyAttendanceInvoiceType.DIRECT,
      staffingCompanyId: null,
      lines: lines.filter((line) => !line.staffingCompanyId),
    },
    ...staffingCompanyIds.map((staffingCompanyId) => ({
  ...
  const invoices = await prisma.$transaction(async (tx) => {
    ...
    for (const group of groups) {
      ...
      const regularHours = sum(group.lines.map((line) => Number(line.regularHours)))
      const overtimeHours = sum(group.lines.map((line) => Number(line.overtimeHours)))
      const rate = invoiceRate(group.type)
      const totalAmount = Number((totalHours * rate).toFixed(2))

      created.push(
        await tx.weeklyAttendanceInvoice.create({
          data: {
  ...
  await audit("CREATE_WEEKLY_ATTENDANCE_INVOICES", batch, {
...
export async function markWeeklyAttendanceInvoicePaid(id: string, user: CurrentUser) {
  ...
  const updated = await prisma.weeklyAttendanceInvoice.update({
    where: { id },
    data: {
      status: WeeklyAttendanceInvoiceStatus.PAID,
      paidAt: new Date(),
    },
  })
...
function invoiceRate(type: WeeklyAttendanceInvoiceType) {
  switch (type) {
    case WeeklyAttendanceInvoiceType.DIRECT:
      return DEFAULT_DIRECT_INVOICE_RATE
    case WeeklyAttendanceInvoiceType.STAFFING:
      return DEFAULT_STAFFING_INVOICE_RATE
    case WeeklyAttendanceInvoiceType.CONSOLIDATED:
      return DEFAULT_CONSOLIDATED_INVOICE_RATE
    default:
      return DEFAULT_DIRECT_INVOICE_RATE
  }
}
